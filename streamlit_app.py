...
def highlight_outdated(s):
    return ['background-color: red']*len(s) if datetime.strptime(s['Tanggal Keluar'], "%Y-%m-%d").date() > date.today() else ['background-color: black']*len(s)

st.dataframe(df[1:].style.apply(highlight_outdated, axis=1), use_container_width=True)
...
with col2:
    with st.expander("Keluarkan sampel"):
        ...
        if submitted:
            if pic != ' ':
                deleted_row = int(df[df['info']==sampel].index[0])
